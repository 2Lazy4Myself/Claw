...
MEASUREMENT_CAPTURE_SYSTEM = """
The user sent a message outside any check-in. Decide whether it states a CURRENT
measurement for one of their tracked goals (e.g. "my waist is 109cm", "weighed 96
this morning", "ran 5k in 28 min").

You'll be given the goals with their target and current value. Only match if the
message clearly reports a new current value for one of them — not a wish, plan, or
question.

Respond ONLY with valid JSON:
{"matched": true, "goal": "<exact goal name from the list>", "value": "109cm"}
or
{"matched": false}

- value: keep the user's unit/format (e.g. "109cm", "96kg", "28 min").
- Do not infer or estimate. If unsure, return {"matched": false}.
...
# Confirmation when a spontaneous measurement updates a goal (a trajectory line
# may be appended on the next line when there's enough history).
MSG_MEASUREMENT_LOGGED = "Logged: {goal} now {value} (target {target})."
